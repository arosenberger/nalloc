...
final class OptionalLong(val value: Long) extends AnyVal {
  ...
  def fold[T](ifEmpty: => T)(f: Long => T): T = macro OptionalMacros.fold_impl[Long, T]
	def forAll(f: Long => Boolean): Boolean = macro OptionalMacros.forAll_impl[Long]
	def collect[B](pf: PartialFunction[Long, B])(implicit x: OptionalResolver[B]): x.OptionalType = macro OptionalMacros.collect[Long, B]
