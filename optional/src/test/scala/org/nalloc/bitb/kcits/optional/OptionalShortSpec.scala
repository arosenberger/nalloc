...
class OptionalShortSpec extends OptionalTypeSuite {
	...
	property("orElse on the empty value returns the passed in alternative") {
		OptionalShort.empty.orElse(OptionalShort(1)) shouldBe OptionalShort(1)
	}

	property("orElse on non empty values does not evaluate the passed in function") {
		forAll { x: Short =>
			whenever(x != Short.MinValue) {
				OptionalShort(x).orElse(throw new IllegalArgumentException) shouldBe OptionalShort(x)
			}
		}
	}
	...
	property("collect on non empty values evaluates the passed in partial function") {
		forAll { x: Short =>
			whenever(x != Short.MinValue && x > 0) {
				x % 2 match {
					case 0 =>
						val projection = OptionalShort(x).collect {
							case that if that % 2 == 1 => 100
						}

						projection shouldBe OptionalInt.empty

						val projectionWithDefault = OptionalShort(x).collect {
							case that if that % 2 == 1 => 100
							case _ => 200
						}
